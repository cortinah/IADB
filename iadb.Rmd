...
feature_list <- c('lugar','area','sanitario','pared','piso','techo','abastagua','epared','etecho','instlevel')
...
select <- train %>% select(Target, mobilephone, refrig, computer, television, noelec)
...
```{r svm}
model_svm <- svm(Target ~ ., data=features, kernel='radial',cost=2,gamma=0.5)
...
y <- features$Target 
predictions <- features$pred
conf <- confusionMatrix(predictions, y, mode = "prec_recall")
